...
public class Exercise_2 {
    ...
    private static class VProg extends AbstractFunction3<Long,Integer,Integer,Integer> implements Serializable {
        @Override
        public Integer apply(Long nodeID, Integer nodeValue, Integer message) {
           System.out.println("[ VProg ] nodeID: '" +  nodeID +  "' nodeValue: '" +  nodeValue + "' message: '" + message + "'" );

            if (message.equals(MVALUE)) {
               System.out.println("[ VProg ] State 0 -> nodeID: '" +  nodeID +  "'");
                return nodeValue;
            } else {
               System.out.println("[ VProg ] nodeID: '" +  nodeID +  "' '" + Math.min(nodeValue, message) + "' value");
                return Math.min(nodeValue, message);
            }
        }
    }
    ...
    private static class sendMsg extends AbstractFunction1<EdgeTriplet<Integer,Integer>, Iterator<Tuple2<Object,Integer>>> implements Serializable {
        ...
        public Iterator<Tuple2<Object, Integer>> apply(EdgeTriplet<Integer, Integer> triplet) {
            ...
            Integer sourceNode = triplet.srcAttr();
            Integer destinationNode = triplet.dstAttr();

            if ( sourceNode.equals(MVALUE) ) {
               System.out.println("[ sendMsg ] srcId: '" +  srcId +  " [" + sourceNode + "]'  to dstId: '" + dstId + " [" + destinationNode + "]'");
                return JavaConverters.asScalaIteratorConverter(new ArrayList<Tuple2<Object,Integer>>().iterator()).asScala();
            } else {
                Integer message = sourceNode + weight;
               System.out.println("[ sendMsg ] srcId: '" +  srcId +  " [" + sourceNode + "]' send '" + message + "' to dstId: '" + dstId + " [" + destinationNode + "]'");
	...
	public static void shortestPaths(JavaSparkContext ctx) {
        ...
        ops.pregel(Integer.MAX_VALUE,
            ...
            .vertices()
            .toJavaRDD()
                .sortBy(node -> ((Tuple2<Object, Integer>) node)._1, true, 0)
                .foreach(v -> {
                Tuple2<Object,Integer> node = (Tuple2<Object,Integer>)v;
                System.out.println("Minimum cost to get from "+labels.get(1l)+" to "+labels.get(node._1)+" is "+node._2);
            });
	}
